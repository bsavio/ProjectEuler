...
def print_triangle(triangle_to_print):
    for row in triangle_to_print:
        print(row)
...
def find_largest_triangle_path_sum(triangle):
    row_count = len(triangle) - 1
    for i in reversed(range(0, row_count)):
        triangle[i] = consolidate_triangle_rows(triangle[i], triangle[i+1])
        del triangle[-1]
...
print_triangle(large_triangle)
result = find_largest_triangle_path_sum(large_triangle)
print(result)
